...
def findSTM(r0, v0, rf, vf, dt):
    ...
    Y0 = np.block(
        [
            [
                sr0,
                -1.0 * np.matmul((np.matmul(sr0, sv0) + sh), B),
                -1.0 * np.transpose([r0]),
            ],
            [
                sv0,
                np.matmul(
                    mu / r0Mag**3 * np.matmul(sr0, sr0) - np.matmul(sv0, sv0), B
                ),
                np.transpose([v0]) / 2.0,
            ],
        ]
    ...
    Yf = np.block(
        [
            [
                srf,
                -1.0 * np.matmul((np.matmul(srf, svf) + sh), B),
                np.transpose([-1.0 * rf + 3.0 / 2.0 * dt * vf]),
            ],
            [
                svf,
                np.matmul(
                    mu / rfMag**3 * np.matmul(srf, srf) - np.matmul(svf, svf), B
                ),
                np.transpose([vf / 2.0 - 3.0 / 2.0 * mu / rfMag**3 * dt * rf]),
            ],
        ]
    )
