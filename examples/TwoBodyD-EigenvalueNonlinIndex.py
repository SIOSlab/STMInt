...
def calc_error(stm, transfer_time, x_0, perturbation):
    ...
    return np.linalg.norm(((r_f_pert - r_f_ref) - np.matmul(stm, delta_x_0)[:3]), ord=2)/np.linalg.norm(np.matmul(stm, delta_x_0)[:3], 2)
...
for i in range(0, 20):
    ...
    m_1yvals.append(0.5 * pow(r, 1) * m_1norm * 100.)

    # Method 2: Making an educated guess at the maximum error.
    m_2yvals.append(calc_error(stm, transfer_time, x_0, r * sttArgMax) * 100.)
...
model3.set_ylabel("DEMoN (%)", fontsize=16)
